...
const QuizMe: React.FC = () => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    if (result.length === 0) {
        setError("Sorry, I couldn't generate a quiz for this topic. Please try another one.");
    } else {
        setQuestions(result);
    }
  ...
  const getButtonClass = (questionIndex: number, option: string) => {
    if (!submitted) {
      return userAnswers[questionIndex] === option ? 'bg-blue-200 border-primary' : 'bg-white hover:bg-gray-100';
    }
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-xl font-semibold text-center text-text-primary">Test Your Knowledge</h3>
      <p className="text-center text-text-secondary">Challenge yourself with a quick quiz on any subject.</p>
        ...
        <div className="mt-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {q.options.map((option, oIndex) => (
                  <button
                    ...
                    disabled={submitted}
                    className={`p-2 w-full text-left border rounded-lg transition-colors ${getButtonClass(qIndex, option)}`}
                  >
                    {option}
                  </button>
                ))}
          ...
          {!submitted ? (
            <div className="text-center">
                <button onClick={handleSubmit} className="bg-green-500 text-white font-bold px-8 py-3 rounded-lg hover:bg-green-600 transition-colors">
                    Submit Answers
                </button>
            </div>
          ) : (
            <div className="text-center p-6 bg-blue-100 rounded-lg">
                ...
                <p className="text-xl mt-2">Your score: <span className="font-bold">{score} / {questions.length}</span></p>
                <button onClick={handleGenerate} className="mt-4 bg-primary text-white font-semibold px-6 py-2 rounded-lg hover:bg-blue-600">
