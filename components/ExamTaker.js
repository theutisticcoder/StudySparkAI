...
const ExamTaker = ({ exam, onSubmit, timeMultiplier }) => {
  const [mcqAnswers, setMcqAnswers] = useState({})
  const [frqAnswers, setFrqAnswers] = useState({})
  const [timeLeft, setTimeLeft] = useState(
    exam.recommendedTime * timeMultiplier * 60
  )
  ...
  const handleMcqChange = (qIndex, optionKey) => {
    setMcqAnswers(prev => ({ ...prev, [qIndex]: optionKey }))
  }

  const handleFrqChange = (qIndex, text) => {
    ...
    <div className="space-y-8">
      <div className="sticky top-16 bg-white/80 backdrop-blur-sm py-4 border-b z-10 text-center">
        <h3 className="text-2xl font-bold text-primary">
          AP {exam.subject} Practice Exam
        </h3>
        <div
          className={`mt-2 text-2xl font-mono ${
            timeLeft < 300 ? "text-red-500" : "text-text-primary"
          }`}
        >
      ...
      <div className="space-y-6">
        <h4 className="text-xl font-semibold border-b pb-2">
          Multiple-Choice Questions ({exam.mcqs.length} questions)
        </h4>
        {exam.mcqs.map((q, qIndex) => (
          ...
          >
            <p className="font-semibold mb-3 whitespace-pre-wrap">
              {qIndex + 1}. {q.question}
            </p>
            <div className="space-y-2">
              {Object.entries(q.options).map(([key, value]) => (
                <label
                  key={key}
                  className="flex items-center p-2 border rounded-lg cursor-pointer transition-colors bg-white hover:bg-gray-100 has-[:checked]:bg-blue-100 has-[:checked]:border-primary"
                >
                  <input
                    type="radio"
                    name={`mcq-${qIndex}`}
      ...
      <div className="space-y-6">
        <h4 className="text-xl font-semibold border-b pb-2">
          Free-Response Questions ({exam.frqs.length} questions)
        </h4>
