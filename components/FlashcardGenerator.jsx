...
const FlashcardGenerator = () => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    if (result.length === 0) {
      setError(
        "Sorry, I couldn't generate flashcards for this topic. Please try another one."
      )
    } else {
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-center text-text-secondary">
        Enter a subject to generate a set of flashcards for active recall
        practice.
      </p>
      ...
      {flashcards.length > 0 && (
        <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {flashcards.map((card, index) => (
            ...
            >
              <div
                className={`relative w-full h-48 rounded-lg shadow-md cursor-pointer transition-transform duration-500 transform-style-3d ${
