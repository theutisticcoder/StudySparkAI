...
const ApPracticePlanner = () => {
  const [selectedSubject, setSelectedSubject] = useState(null)
  const [plan, setPlan] = useState("")
  const [isLoading, setIsLoading] = useState(false)
  const [error, setError] = useState(null)

  const handleGeneratePlan = useCallback(async subject => {
    ...
    <section id="ap-planner">
      <div className="text-center mb-8">
        <h2 className="text-3xl font-bold text-text-primary">
          AP Exam Practice Plans
        </h2>
        <p className="text-md text-text-secondary max-w-2xl mx-auto mt-2">
          Select an AP subject to get a personalized 4-week study plan from our
          AI.
        </p>
      </div>
      <Card>
          ...
          {plan && (
            <div className="p-6 bg-gray-50 border border-gray-200 rounded-lg">
              <h3 className="text-2xl font-bold text-primary mb-4">
                4-Week Study Plan for AP {selectedSubject}
              </h3>
